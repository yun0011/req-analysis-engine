...
| `global_call_graph.json` | 1 완료 | 파일 존재 + `registry` 키 존재 |
| `vue_output/api_map.json` | 1 완료 (선택) | 클라이언트 코드 존재할 때만 확인 |
| `vue_output/routes.json` | 1 완료 (선택) | 동상 |
| `vue_output/component_map.json` | 1 완료 (선택) | 동상 |
| `vue_output/client_behaviors.json` | 1 완료 (선택) | 동상 |
| `vue_output/display_labels.json` | 1 완료 (선택) | 동상 |
| `vue_output/ui_structure.json` | 1 완료 (선택) | 동상 |
...
통합 스크립트로 Phase 1 전체를 실행한다:

```bash
# 전체 실행 (백엔드 + 클라이언트 + Call Graph)
python extract.py
...
1. 백엔드 AST 추출: `python extract.py --backend`
2. 클라이언트 정적 분석: `python extract.py --client` (선택)
3. Call Graph 생성: `python extract.py --graph`

**완료 후 검토 항목:**

```
✓ analysis_output/ 파일 수: {N}개
✓ global_call_graph.json 내부 함수 수: {N}개 / 외부 호출 필터링: {N}개 제거
✓ 클라이언트 분석 (클라이언트 코드가 있을 때):
  - vue_output/api_map.json:         API 메서드 → EP 매핑 {N}개
  - vue_output/routes.json:          라우트 {N}개, 권한 분기 포함
  - vue_output/component_map.json:   컴포넌트 {N}개 → API 호출 체인
  - vue_output/client_behaviors.json: 행동 패턴 (폴링 {N}, 확인 가드 {N}, disabled 조건 {N})
  - vue_output/display_labels.json:  상태 코드 → 표시 텍스트 도메인 {N}개
  - vue_output/ui_structure.json:    페이지 {N}개 (컬럼 있는 페이지 {N}, 폼 있는 페이지 {N})
```

**중단 조건:**
- `analysis_output/`가 비어있음 → AST 추출 스크립트 실행 오류 진단
- `global_call_graph.json`의 `registry`가 비어있음 → Call Graph 생성 스크립트 오류 진단
...
Rule Record 생성 시 `vue_output/routes.json`의 권한 설정을 참조해 `trigger.actor`를 결정한다.
...
대상: `vue_output/component_map.json`에서 라우트 경로가 있고 API를 호출하는 페이지 단위 컴포넌트.
...
산출물:
  ...
  global_call_graph.json                  — Call Graph (내부 함수 N개)
  vue_output/api_map.json                 — API 메서드 → EP 매핑 (N개)
  vue_output/routes.json                  — 라우트 + 권한 (N개)
  vue_output/component_map.json           — 컴포넌트 → API 호출 체인 (N개)
  vue_output/client_behaviors.json        — 클라이언트 행동 패턴
  vue_output/display_labels.json          — 상태 코드 → 표시 텍스트
  vue_output/ui_structure.json            — 페이지별 UI 구조 (N개)
